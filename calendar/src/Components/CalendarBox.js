...
const reqNodeServer = async function (_method, _url, _params) {
    let dataFromServer = await axios({//token
        method: _method,
        url: _url,
        params: _params
    })
...
export default function CalendarBox(props) {
    ...
    async function getMemoFun() {
        setGetMemo(await reqNodeServer('GET', `${headAddress}/getMemo`, ''));
    }
    useEffect(() => {
        getMemoFun();
    }, [])
    ...
    const getMonthDay = useCallback(() => {
        ...
        let dayIndex = [];
        for (let i = 0; i < 42; i++) {
            let yearFirst = new Date(todayYear, todayMonth - 1, 1 - new Date(todayYear, todayMonth - 1).getDay() + i).getFullYear();
            let monthFirst = new Date(todayYear, todayMonth - 1, 1 - new Date(todayYear, todayMonth - 1).getDay() + i).getMonth() + 1;
            let dateFirst = new Date(todayYear, todayMonth - 1, 1 - new Date(todayYear, todayMonth - 1).getDay() + i).getDate();
    ...
    return (
        <react.Fragment>
            ...
            <div className='calendar-page-btn'>
                월 이동버튼 영역
            </div>
